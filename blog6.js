if(!window.hasOwnProperty("aiExecuted")){
  ...
  function ChucklePostAI(AI_option) {
	...
	function MAIN(AI_option) {
	  ...
	  //发送请求获得简介
	  async function getGptResponse(content, i = false) {
		if (!tlKey) {
		...
		if (i) {
		  try {
			response = await fetch('https://blogsummary.xin-sun.workers.dev/api/summary/?token=12345', {
			  signal: signal,
			  method: "POST",
			  headers: {
				"Content-Type": "application/json",
		  ...
		  const apiUrl = "https://new.csun.site/v1/chat/completions";
		  try {
