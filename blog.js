...
var tianliGPT_postSelector = '.article-content.markdown-body';
...
function insertAIDiv ( selector )
{
  ...
  <g id="&#x673A;&#x5668;&#x4EBA;" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
	...
	aiExplanationDiv.innerHTML = '生成中...' +
...
var tianliGPT = {
	...
	fetchTianliGPT: async function ( content )
	{
		...
		try
		{
			...
			else
			{
				...
				{
					document.querySelectorAll( '.post-TianliGPT' )
						.forEach( el =>
						{
	...
	aiShowAnimation: function ( text )
	{
		...
		if ( typeof tianliGPT_typingAnimate !== "undefined" && !
		...
		const animate = () =>
		{
			if ( currentIndex < text.length && animationRunning )
			{
				...
				const letter = text.slice( currentIndex,
					currentIndex + 1 );
				const isPunctuation = /[，。！、？,.!?]/.test( letter );
				const delay = isPunctuation ? typingDelay *
					punctuationDelayMultiplier : typingDelay;
				if ( timeDiff >= delay )
				{
					...
					{
						element.innerHTML = text.slice( 0,
								currentIndex ) +
							'<span class="blinking-cursor"></span>';
					}
					else
...
function checkURLAndRun ()
{
	...
	try
	{
		...
		{
			console.log( "因为不符合自定义的链接规则，我决定不执行摘要功能。" );
		}
	}
	catch ( error )
	{
		console.error( "我没有看懂你编写的自定义链接规则，所以我决定不执行摘要功能", error );
	}
}
